...
router.post('/crear', requireRole(...ROLES_CREAR), async (req, res) => {
  ...
  try {
    ...
    try {
        const { rows: usuarios } = await db.query("SELECT email FROM users WHERE email IS NOT NULL AND email != ''");
        const listaCorreos = usuarios.map(u => u.email);

        if (listaCorreos.length > 0) {
            let adjuntosCorreo = [];
            try { if (adjuntos_data) adjuntosCorreo = JSON.parse(adjuntos_data); } catch(e) {}
            const imagenesCorreo = adjuntosCorreo.filter(a => a.tipo === 'image' || (a.resource_type === 'image' && a.tipo !== 'video'));

            const imagenPortadaHtml = imagen_portada
              ? `<img src="${imagen_portada}" alt="Portada" style="width:100%; max-height:380px; object-fit:cover; display:block; margin-bottom:0;">`
              : '';

            const galeriaHtml = imagenesCorreo.length > 0
              ? `<div style="margin: 20px 0 10px 0;">
                   <p style="color:#003a70; font-weight:700; font-size:15px; margin:0 0 10px 0;">Imágenes adjuntas</p>
                   <div style="text-align: left;">
                     ${imagenesCorreo.map(img => `<a href="${img.url}" target="_blank" style="display:inline-block; margin: 4px; vertical-align: top;"><img src="${img.url}" alt="${img.nombre || 'Imagen'}" style="width: 60px; height: auto; border-radius:6px; border:1px solid #e2e8f0; display:block;"></a>`).join('')}
                   </div>
                 </div>`
              : '';
            ...
            const htmlCorreo = `
                <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; border: 1px solid #e0e0e0; border-radius: 8px; overflow: hidden;">
                    ...
                    <div style="padding: 25px; background-color: #ffffff;">
                        <h3 style="color: #003a70; margin-top: 0; font-size: 22px;">${titulo}</h3>
                        ${subtitulo ? `<p style="color: #475569; font-size: 16px; font-weight: bold;">${subtitulo}</p>` : ''}
                        ...
                        <div style="text-align: center; margin-top: 30px; margin-bottom: 10px;">
                            <a href="${process.env.APP_BASE_URL || 'http://localhost:3000'}/noticias/${noticiaId}" 
                    ...
                    <div style="background-color: #f8fafc; padding: 15px; text-align: center; border-top: 1px solid #e0e0e0;">
                        <p style="color: #94a3b8; font-size: 12px; margin: 0;">Este es un mensaje automático de la Intranet Transworld, por favor no respondas a este correo.</p>
                    </div>
                </div>
            `;
        ...
        console.error("Error al enviar correos de notificación de noticia:", errCorreo);
    }
